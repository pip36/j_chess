...
function Chess(){
	...
	var setUpBoard = function(){
	    ...
	    $('#chess').append("<ul class='moves-list'> </ul> <button id='previous'> < </button> <button id='next'> > </button>");
	    $('.moves-list').append("<li> </li>");
	    $('tr:nth-child(even) td:nth-child(odd)').css("background-color", "grey");
	   	$('tr:nth-child(odd) td:nth-child(even)').css("background-color", "grey");
	}
...
//**** needs edge cases for differentiating between knights/rooks moving to a square where both are possible *****//
	...
	this.getBoardString = function(){
		let result = "";
		let boardData = game.currentBoardState;
		for(var x = 0; x < 8; x++){
			for(var y = 0; y < 8; y++){
				$('#' + (8-x) + " td").eq(y).html(CHESS_CHARACTERS[boardData[x][y]]);
				result += boardData[x][y];
			}
		}
		return result;
	}
